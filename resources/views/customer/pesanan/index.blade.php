...
@section('content')
    ...
    {{-- Header Hero --}}
    <div
        class=" max-w-6xl mx-auto relative overflow-hidden rounded-3xl border border-white/60 bg-gradient-to-tr from-indigo-500/20 via-indigo-300/10 to-transparent p-6">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
            ...
            <a href="{{ route('pesanan.buat') }}"
                ...
                <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-width="2" d="M12 5v14m7-7H5" />
                </svg>
                Pesan
            </a>
        </div>
        ...
        {{-- Desktop: Tabel --}}
        <div
            class="max-w-6xl mx-auto mt-4 hidden md:block rounded-2xl border border-white/60 bg-white/80 backdrop-blur shadow-sm overflow-hidden">
            @if ($items->isEmpty())
                            ...
                            @foreach ($items as $p)
                                <tr class="hover:bg-white/50 transition">
                                    <td class="px-4 py-3">
                                        ...
                                        <div class="text-[11px] text-slate-400">{{ $p->created_at->format('d M Y H:i') }}
                                        </div>
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        <span
                                            class="inline-flex items-center rounded-full px-2 py-0.5 text-xs ring-1 {{ $badge($p->status) }}">
                                            {{ strtoupper($p->status) }}
                                        </span>
                                    </td>
        ...
        <div class="max-w-6xl mx-auto mt-4 space-y-3 lg:hidden">
            @forelse($items as $p)
                <div
                    class="relative rounded-2xl border border-white/60 bg-white/85 backdrop-blur p-4 shadow-sm
                before:absolute before:inset-y-0 before:left-0 before:w-1 before:rounded-l-2xl {{ $accent($p->status) }}">
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <span
                            class="inline-flex items-center rounded-full px-2 py-0.5 text-[11px] ring-1 {{ $badge($p->status) }}">
                            {{ strtoupper($p->status) }}
                        </span>
                    </div>
