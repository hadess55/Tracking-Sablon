...
@section('content')
    ...
    <div class="fixed inset-0 -z-10">
        ...
        <div
            class="absolute inset-0 bg-[radial-gradient(rgba(99,102,241,.12)_1px,transparent_1px)] [background-size:16px_16px]">
        </div>
    </div>
    ...
    <div class="mx-auto max-w-3xl">
        <form method="GET" action="{{ url('tracking') }}"
            ...
            <input type="text" name="nomor"
                ...
                placeholder="Masukkan nomor resi…" value="{{ old('nomor', $nomor ?? '') }}" autocomplete="off" required />
        ...
        <div class="max-w-5xl mx-auto mt-8">
            @if ($produksi)
                ...
                "
                    ...
                    <div class="mt-8">
                        <div class="text-xs uppercase tracking-wide text-slate-500 mb-3">STATUS</div>

                        <ul role="list" class="space-y-5">
                            @forelse($produksi->logs as $r)
                                @php
                                    $t = Str::title($r->tahapan);
                                    $dotCol = $dot[$t] ?? 'bg-indigo-500';
                                ...
                                <li class="relative pl-7">
                                    @if (!$loop->last)
                                        <span class="absolute left-1.5 top-5 -ml-px h-full w-px bg-slate-200"></span>
                                    @endif

                                    <span
                                        class="absolute left-0 top-1.5 h-3 w-3 rounded-full ring-4 ring-white {{ $isLatest ? $dotCol : 'bg-slate-300' }}"></span>

                                    <div>
                                        <h3 class="mt-1 text-md font-semibold text-slate-900">
                                            {{ $r->status_key }}
                                        </h3>
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                <div class="mb-3 inline-flex h-10 w-10 items-center justify-center rounded-xl bg-indigo-50 text-indigo-600">
                    <svg class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                        <path d="M12 2L2 7l10 5 10-5-10-5Zm0 7l10 5-10 5L2 14l10-5Z" />
                    </svg>
                </div>
        ...
        <div x-data="{ open: 1 }" class="space-y-3">
            ...
            <div class="rounded-xl border border-slate-200 bg-white/80 shadow-sm">
                <button type="button" class="w-full flex items-center justify-between px-5 py-4"
                    ...
                    <svg class="h-5 w-5 text-slate-500 transition-transform" :class="open === 1 ? 'rotate-180' : ''"
            ...
            <div class="rounded-xl border border-slate-200 bg-white/80 shadow-sm">
                <button type="button" class="w-full flex items-center justify-between px-5 py-4"
                    ...
                    <svg class="h-5 w-5 text-slate-500 transition-transform" :class="open === 2 ? 'rotate-180' : ''"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path stroke-linecap="round" stroke-width="2" d="m6 9 6 6 6-6" />
                    </svg>
                </button>
                <div x-show="open === 2" x-transition class="px-5 pb-5 text-slate-600">
                    Pastikan format nomor benar dan tanpa spasi. Jika masih tidak muncul, hubungi admin agar data Anda
                    di-sinkronkan.
                </div>
            </div>
            ...
            <div class="rounded-xl border border-slate-200 bg-white/80 shadow-sm">
                <button type="button" class="w-full flex items-center justify-between px-5 py-4"
                    ...
                    <svg class="h-5 w-5 text-slate-500 transition-transform" :class="open === 3 ? 'rotate-180' : ''"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path stroke-linecap="round" stroke-width="2" d="m6 9 6 6 6-6" />
                    </svg>
                </button>
            ...
            <div class="rounded-xl border border-slate-200 bg-white/80 shadow-sm">
                <button type="button" class="w-full flex items-center justify-between px-5 py-4"
                    ...
                    <svg class="h-5 w-5 text-slate-500 transition-transform" :class="open === 4 ? 'rotate-180' : ''"
                        viewBox="0 0 24 24" fill="none" stroke="currentColor">
                        <path stroke-linecap="round" stroke-width="2" d="m6 9 6 6 6-6" />
                    </svg>
                </button>
